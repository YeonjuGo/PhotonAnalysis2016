...
void compare_bkgShape(TString coll="pp"){

    const char* fname_1;
    const char* fname_2;
    if(coll=="pp"){ 
        fname_1="/home/goyeonju/CMS/Files/photon2016/2015-Data-promptRECO-photonSkims_pp-photonHLTFilter-v0-HiForest.root";
        fname_2="/home/goyeonju/CMS/Files/photon2016/2015-PP-MC_Pythia8_EmEnrDijet30_pp502_TuneCUETP8M1.root";
    } else if(coll=="pbpb"){
    ...
    TTree* t1_hi = (TTree*) f1 -> Get("skimanalysis/HltTree");
    t1->AddFriend(t1_hi);
    TFile* f2 = new TFile(fname_2);
    TTree* t2 = (TTree*) f2 -> Get("ggHiNtuplizerGED/EventTree");
    TTree* t2_hi = (TTree*) f2 -> Get("skimanalysis/HltTree");
    t2->AddFriend(t2_hi);
    ...
    for(Int_t i = 0; i < nPtBin; ++i) {
        ...
        TCut dataTotCut = dataCut_pp && sidebandIsolation && ptCut && etaCut;
        cout << dataTotCut.GetTitle() << endl;
        TCut mcTotCut_ = mcBkgIsolation && ptCut && etaCut;
        cout << mcTotCut_.GetTitle() << endl;
        //TCut mcTotCut_ = ptCut && etaCut && mcBkgIsolation; 
      
       //  TCut dataTotCut = "";
       // TCut mcTotCut_ =""; 
      
        const char* cap = Form("sumIso10to20_pt%dto%d",(int)ptBins[i],(int)ptBins[i+1]);
        //compareTwo(t1, t2, "phoEt",50, 60, 250,dataTotCut, mcTotCut,cap);
        //compareTwo(t1, t2, "phoR9",50, 0, 1,dataTotCut, mcTotCut,cap);
        compareTwo(t1, t2, "phoSigmaIEtaIEta",50, 0, 0.03,dataTotCut, mcTotCut_,cap);
...
*/
    ...
    legStyle(l1);
    l1->AddEntry(h1,"pp DATA","pl");
    l1->AddEntry(h1,"EmEnrDijet","l");
    l1->Draw();

	c->cd(2);
	h1->Divide(h2);
	h1->SetYTitle("pp DATA / Em-Enriched Dijet");
